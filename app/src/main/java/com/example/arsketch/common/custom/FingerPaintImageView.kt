package com.example.arsketch.common.custom
...
class FingerPaintImageView @JvmOverloads constructor(
    ...
    private var bgPaint = Paint(1)
    private var mRectF = RectF()
    private var forwardList = mutableListOf<Pair<Path, Paint>>()
    ...
    private fun handleTouchStart(event: MotionEvent) {
        ...
        val imageBounds = RectF(
            xTranslation,
            yTranslation,
            xTranslation + width * scale,
            yTranslation + height * scale
        )
    ...
    fun modeEraser(mStrokeWidth: Float) {
        pathPaint.strokeWidth = mStrokeWidth
    ...
    fun modeDraw(mStrokeWidth: Float) {
        pathPaint.strokeWidth = mStrokeWidth
        pathPaint.setXfermode(
            PorterDuffXfermode(
    ...
    fun setBG(color: String) {
        bgPaint.color = Color.parseColor(color)
        invalidate()
    }
    ...
    private fun handleTouchMove(event: MotionEvent) {
        ...
        val xPos = event.x.coerceIn(xTranslation, xTranslation + width * scale)
        val yPos = event.y.coerceIn(yTranslation, yTranslation + height * scale)
     ...
     */
    fun undo() {
        val item = paths.takeIf { it.isNotEmpty() }?.removeAt(paths.lastIndex)
        item?.let {
            forwardList.add(it)
        }
        countDrawn--
    ...
    fun forward() {
        if (forwardList.isNotEmpty()) {
            paths.add(forwardList.toMutableList()[0])
            forwardList.removeAt(0)
            countDrawn++
            invalidate()
        }
    }
